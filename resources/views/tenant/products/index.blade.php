...
>
    ...
    <div class="text-sm text-slate-500 dark:text-slate-400">
        <template x-if="search || category || stockFilter">
            <span>
                Mostrando <span class="font-semibold text-slate-700 dark:text-slate-200" x-text="shownCount"></span>
                de <span class="font-semibold text-slate-700 dark:text-slate-200" x-text="totalCount"></span> productos
            </span>
        </template>
        <template x-if="!search && !category && !stockFilter">
            <span>
                Mostrando <span class="font-semibold text-slate-700 dark:text-slate-200" x-text="shownCount"></span> productos
            </span>
        </template>
    </div>
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8 max-w-6xl w-full">
            ...
            <template x-for="p in filtered" :key="p.id">
                <div class="bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700
                    ...
                    <div class="flex justify-center mb-4">
                        <template x-if="p.image">
                            <img :src="`/storage/${p.image}`"
                                 class="w-28 h-28 object-cover rounded-xl border border-slate-300 dark:border-slate-600 bg-white">
                        </template>
                        <template x-if="!p.image">
                    ...
                    <h2 class="text-lg font-semibold text-slate-800 dark:text-slate-100 text-center" x-text="p.producto"></h2>
                    <p class="text-xs text-slate-500 dark:text-slate-400 text-center mt-1" x-text="p.codigo ?? '—'"></p>

                    <hr class="border-slate-200 dark:border-slate-700 my-4">

                    <div class="flex justify-between items-center text-sm">
                        <span class="text-slate-500 dark:text-slate-400">PVP</span>
                        <span class="font-semibold text-teal-600 dark:text-teal-400" x-text="`${p.pvp} €`"></span>
                    </div>

                    <div class="flex justify-center mt-4">
                        <span class="px-4 py-1 rounded-full text-xs font-semibold"
                              :class="stockClasses(p.stock)"
                              :title="stockTitle(p.stock)">
                            Stock: <span x-text="p.stock"></span>
                        </span>
                    </div>

                    {{-- BOTONES GRID (SIN ELIMINAR NADA) --}}
                    <div class="flex justify-center gap-3 mt-6">
                        ...
                        <button
                            ...
                            class="w-10 h-10 rounded-xl
                                   ...
                                   dark:bg-teal-600 dark:text-white dark:border-transparent
                        ...
                        <a :href="`/tenant/products/${p.id}/edit`"
                           class="w-10 h-10 rounded-xl
                                  ...
                                  dark:bg-slate-600 dark:text-white dark:border-transparent
                ...
                <tbody>
                    <template x-for="p in filtered" :key="p.id">
                        <tr class="border-t border-slate-200 dark:border-slate-700
                            ...
                            <td class="px-4 py-3 text-teal-600 dark:text-teal-400" x-text="`${p.pvp} €`"></td>
